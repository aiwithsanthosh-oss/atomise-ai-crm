...
const fieldLabel =
  "block text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70 mb-1.5";
const fieldInput =
...
const fieldInputError =
  "w-full h-11 card-elevated border border-red-500 text-foreground text-sm font-medium rounded-xl px-3 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500/20 placeholder:text-muted-foreground/50 transition-all";
...
const isEmptyCustomValue = (field: CustomFieldDef, value: any) => {
...
const isValidEmailValue = (value: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);
...
const validateCustomFieldValues = (
  ...
  for (const field of fields) {
    ...
    const hasValue = !isEmptyCustomValue(field, rawValue);
    ...
    if (field.field_type === "date") {
      if (!isValidDateValue(String(rawValue))) {
    ...
    if (field.field_type === "dropdown") {
      ...
      if (!options.includes(String(rawValue))) {
        errors[field.field_key] = `Please select a valid ${field.label.toLowerCase()}`;
      }
      continue;
    }
    ...
    if (field.field_type === "multiselect") {
      ...
      if (!Array.isArray(rawValue) || selected.some((item) => !options.includes(item))) {
        errors[field.field_key] = `Please choose valid ${field.label.toLowerCase()} options`;
      }
      continue;
    }
    ...
    if (field.field_type === "checkbox") {
      if (typeof rawValue !== "boolean") {
        errors[field.field_key] = `${field.label} must be true or false`;
      }
      continue;
    }

    if (field.field_type === "url") {
      if (!isValidUrlValue(String(rawValue).trim())) {
        errors[field.field_key] = `${field.label} must be a valid URL`;
      }
      continue;
    }

    if (field.field_type === "email") {
      if (!isValidEmailValue(String(rawValue).trim())) {
        errors[field.field_key] = `${field.label} must be a valid email address`;
      }
      continue;
    }

    if (field.field_type === "phone") {
      if (!isValidPhoneValue(String(rawValue).trim())) {
        errors[field.field_key] = `${field.label} must be a valid phone number`;
      }
      continue;
    }
  }
...
function CustomFieldsBlock({
  ...
  return (
    <div className="space-y-4">
      ...
      {fields.map((field) => {
        const val = customValues[field.field_key] ?? (field.field_type === "multiselect" ? [] : "");
        const labelText = field.is_required ? `${field.label} *` : field.label;
          ...
          return (
            <div key={field.id}>
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => onChange(field.field_key, !val)}
                  className={`relative h-5 w-9 rounded-full transition-colors ${val ? "bg-primary" : "bg-muted"}`}
                >
                  <span
                ...
                <label className="text-sm font-medium text-foreground/80">{labelText}</label>
              </div>
        ...
        if (field.field_type === "dropdown") {
          return (
            <Field key={field.id} label={labelText}>
              <Select value={val || ""} onValueChange={(v) => onChange(field.field_key, v)}>
                <SelectTrigger
                  className={`h-11 card-elevated border text-foreground rounded-xl text-sm font-medium ${
        ...
        if (field.field_type === "multiselect") {
          ...
          return (
            <Field key={field.id} label={labelText}>
              <div className={`flex flex-wrap gap-2 mt-1 rounded-xl ${error ? "ring-1 ring-red-500/40 p-2" : ""}`}>
                {(field.options || []).map((opt) => (
                  <button
                    key={opt}
                    type="button"
        ...
        if (field.field_type === "textarea") {
          return (
            <Field key={field.id} label={labelText}>
              <textarea
                rows={3}
                className={`w-full card-elevated border text-foreground text-sm font-medium rounded-xl px-3 py-2.5 focus:outline-none resize-none placeholder:text-muted-foreground/50 transition-all ${
                  error
          ...
          return (
            <Field key={field.id} label={labelText}>
              <input
                ...
                value={val}
                onChange={(e) => onChange(field.field_key, e.target.value)}
                className={`w-full h-11 card-elevated border text-foreground text-sm font-medium rounded-xl px-3 [color-scheme:light] dark:[color-scheme:dark] transition-all ${
                  error
                    ? "border-red-500 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500/20"
                    : "border-border focus:outline-none focus:border-primary/60"
                }`}
              />
              {error && <p className="text-xs text-red-400 font-medium mt-1">{error}</p>}
            </Field>
          ...
          return (
            <Field key={field.id} label={labelText}>
              <input
                type="text"
                inputMode="decimal"
                className={error ? fieldInputError : fieldInput}
                placeholder={field.placeholder || ""}
                value={val}
                onChange={(e) => onChange(field.field_key, e.target.value)}
              />
...
export default function Contacts() {
  ...
  const [addCustomValues, setAddCustomValues] = useState<Record<string, any>>({});
  ...
  const [addCustomErrors, setAddCustomErrors] = useState<CustomFieldErrors>({});
  const [editCustomErrors, setEditCustomErrors] = useState<CustomFieldErrors>({});
  ...
  const addMutation = useMutation({
    mutationFn: async () => {
      ...
      const customErrors = validateCustomFieldValues(customFieldDefs, addCustomValues);

      if (Object.keys(errors).length > 0 || Object.keys(customErrors).length > 0) {
        setAddErrors(errors);
        setAddCustomErrors(customErrors);
      ...
      if (existing) {
        setAddErrors({ email: "This email is already registered" });
        throw new Error("__validation__");
      }
      ...
      const { error } = await supabase.from("contacts").insert({
        ...
        custom_values: Object.keys(addCustomValues).length > 0 ? addCustomValues : {},
      });
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const customErrors = validateCustomFieldValues(customFieldDefs, editCustomValues);

      if (Object.keys(errors).length > 0 || Object.keys(customErrors).length > 0) {
        setEditErrors(errors);
        setEditCustomErrors(customErrors);
        throw new Error("__validation__");
      }
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          custom_values: editCustomValues,
        })
  ...
  const handleEditClick = (lead: Lead) => {
    ...
    setEditCustomValues(lead.custom_values || {});
    setEditOpen(true);
  };
        ...
        <div className="flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground/70 mt-0.5 font-medium">
              Communication Center · {leads.length} total
            </p>
          </div>
          ...
          {userRole === "admin" && (
            <>
              <Button
                ...
                className="gap-2 bg-primary hover:bg-primary/90 font-bold h-10 px-4 rounded-xl"
              >
              ...
              >
                <DialogContent
                  className="max-w-lg flex flex-col p-0 border border-border rounded-2xl card-bg gap-0 max-h-[90vh]"
                ...
                >
                    ...
                    <div className="px-6 py-5 space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <Field label="Full Name *">
                          <input
                            className={addErrors.name ? fieldInputError : fieldInput}
                            value={form.name}
                            onChange={(e) => {
                        ...
                        <Field label="Email *">
                          <input
                            className={addErrors.email ? fieldInputError : fieldInput}
                            type="email"
                            value={form.email}
                            onChange={(e) => {
                      ...
                      <Field label="Mobile *">
                        <PhoneInput
                          ...
                          onPhoneNumberChange={(n) => {
                            setForm({ ...form, phone: n });
                            setAddErrors((p) => ({ ...p, phone: undefined }));
                          }}
                          numberError={addErrors.phone}
                          variant="form"
                        />
                      </Field>
                      ...
                      <Field label="Tags">
                        <input
                          ...
                          placeholder="Qualified, Hot Lead, Decision Maker"
                        />
                      </Field>
                      ...
                      <CustomFieldsBlock
                        ...
                        customValues={addCustomValues}
                        errors={addCustomErrors}
                        onChange={(key, value) => {
                          setAddCustomValues((prev) => ({ ...prev, [key]: value }));
                          setAddCustomErrors((prev) => {
                            const next = { ...prev };
                            delete next[key];
                            return next;
                          });
                  ...
                  <div className="px-6 py-4 border-t border-border card-elevated">
                    <Button
                      ...
                      className="w-full font-bold h-11 rounded-xl bg-primary hover:bg-primary/90"
                    >
              ...
              <thead className="sticky top-0 z-10 card-bg">
                <tr className="border-b border-border">
                  ...
                  {[
                    ...
                    <th
                      key={label}
                      className={`px-4 py-2.5 text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground/70 ${align === "center" ? "text-center" : "text-left"} ${label === "Actions" ? "pr-6" : ""} ${sortKey ? "cursor-pointer select-none group/th hover:text-primary transition-colors" : ""}`}
                      ...
                      <span className="inline-flex items-center gap-1">
                        ...
                        {sortKey && (
                          <span className="flex flex-col leading-none opacity-40 group-hover/th:opacity-100 transition-opacity">
                            <svg className={`h-2 w-2 ${sortField === sortKey && sortDir === "asc" ? "text-primary opacity-100" : ""}`} viewBox="0 0 8 5" fill="currentColor">
                              <path d="M4 0L8 5H0z" />
                            </svg>
                            <svg className={`h-2 w-2 ${sortField === sortKey && sortDir === "desc" ? "text-primary opacity-100" : ""}`} viewBox="0 0 8 5" fill="currentColor">
                              <path d="M4 5L0 0h8z" />
                            </svg>
                          </span>
                        )}
                    ...
                    <td colSpan={7} className="py-20 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        {!search && userRole === "admin" && (
                          <p className="text-xs text-muted-foreground/50">Click "Add Lead" to get started</p>
                        )}
                      </div>
                    </td>
                  </tr>
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          {lead.phone && (
                            <a href={`tel:${lead.phone}`} className="flex items-center gap-1.5 w-fit group/phone">
                              ...
                              <span className="text-[12px] text-primary font-bold group-hover/phone:text-primary/80 transition-colors">
                                {lead.phone}
                              </span>
                            </a>
                          )}
                        ...
                        <div className="flex justify-center gap-1 opacity-60 group-hover/row:opacity-100 transition-opacity">
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => generateAiSummary(lead)} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-purple-400 hover:bg-purple-500/10 transition-all">
                                <Sparkles className="h-3.5 w-3.5" />
                              </Button>
                            </TooltipTrigger>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" onClick={() => setSelectedLead(lead)} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all">
                                <Clock className="h-3.5 w-3.5" />
                              </Button>
                            </TooltipTrigger>
                          ...
                          {userRole === "admin" && (
                            <>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button variant="ghost" size="icon" onClick={() => handleEditClick(lead)} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all">
                                    <Pencil className="h-3.5 w-3.5" />
                                  </Button>
                                </TooltipTrigger>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button variant="ghost" size="icon" onClick={() => setContactToDelete(lead)} className="h-8 w-8 rounded-lg text-muted-foreground hover:text-red-400 hover:bg-red-500/10 transition-all">
                                    <Trash2 className="h-3.5 w-3.5" />
                                  </Button>
                                </TooltipTrigger>
            ...
            <div className="shrink-0 flex items-center justify-between px-5 py-3 border-t border-border">
              ...
              <div className="flex items-center gap-1">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((item, idx) =>
                    item === "…" ? (
                      <span key={`e-${idx}`} className="h-8 w-6 flex items-center justify-center text-xs text-muted-foreground/50">
                        …
                      </span>
                    ) : (
                      <button
            ...
            <div className="flex-1 overflow-y-auto min-h-0">
              <div className="px-6 py-5 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Full Name *">
                    <input
                      className={editErrors.name ? fieldInputError : fieldInput}
                      value={editForm.name}
                      onChange={(e) => {
                  ...
                  <Field label="Email *">
                    <input
                      className={editErrors.email ? fieldInputError : fieldInput}
                      type="email"
                ...
                <Field label="Mobile *">
                  <PhoneInput
                    ...
                    onPhoneNumberChange={(n) => {
                      setEditForm({ ...editForm, phone: n });
                      setEditErrors((p) => ({ ...p, phone: undefined }));
                    }}
                    numberError={editErrors.phone}
                    variant="form"
                  />
                </Field>
                ...
                <CustomFieldsBlock
                  ...
                  onChange={(key, value) => {
                    setEditCustomValues((prev) => ({ ...prev, [key]: value }));
                    setEditCustomErrors((prev) => {
                      const next = { ...prev };
                      delete next[key];
                      return next;
                    });
        ...
        <AlertDialog open={bulkDeleteOpen} onOpenChange={(o) => { if (!o) setBulkDeleteOpen(false); }}>
          <AlertDialogContent className="card-bg border border-border rounded-2xl">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently remove <span className="font-bold text-foreground">{selectedIds.length} selected contact{selectedIds.length > 1 ? "s" : ""}</span> and all their activity history. This cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel className="rounded-xl" onClick={() => setBulkDeleteOpen(false)}>
                Cancel
              </AlertDialogCancel>
              <AlertDialogAction
        ...
        <AlertDialog open={!!contactToDelete} onOpenChange={(o) => { if (!o) setContactToDelete(null); }}>
          <AlertDialogContent className="card-bg border border-border rounded-2xl">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently remove <span className="font-bold text-foreground">{contactToDelete?.name}</span> and all activity history. This cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
                ...
                <div className="space-y-3 pb-8">
                  ...
                  {activities.map((act) => (
                    <div key={act.id} className="relative bg-muted/30 border border-border rounded-xl p-4 overflow-hidden">
                      ...
                      <p className="text-[13px] text-foreground/75 leading-relaxed font-medium pl-1">
                        {act.content}
                      </p>
                    </div>
                  ))}
